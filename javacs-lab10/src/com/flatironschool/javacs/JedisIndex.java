// TermCounter: maps search term to num of times it appears on a webpage (hash, use jedis.hincrBy)
// Index: maps from search term to set of pages where it appears (set name)
...
public class JedisIndex {
    ...
    String count = jedis.hget(termCounterKey(url), term);
		int numCount = 0;
		if (count != null) numCount = Integer.parseInt(count);
		return numCount;
	}
	...
	public void indexPage(String url, Elements paragraphs) {
				...
				for (String term : terms) {
					...
					jedis.hset(termCounterKey(url), term, Integer.toString(tc.get(term)));
				}
	}
